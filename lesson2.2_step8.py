...
try:
    ...
    input3.send_keys("address@address")
    
    current_dir = os.path.abspath(os.path.dirname(__file__)) 
    file_name = "file.txt"   # получаем путь к директории текущего исполняемого файла 
    file_path = os.path.join(current_dir, file_name)   
    element = browser.find_element_by_css_selector("[type='file']")        # добавляем к этому пути имя файла 
